...
const ResourceImage = props => {
    ...
    /** @magento_update */
    /** @todo this is a hotfix for removing the /cache/%32charstring%/ from the image URL which where mostly incorrect. */
    const regExp = /\/cache(.*?)\/[^\/.]{32}/g;
    const fixedSrc = src.replace(regExp, '');
    const fixedSrcSet = srcSet.replaceAll(regExp, '');
