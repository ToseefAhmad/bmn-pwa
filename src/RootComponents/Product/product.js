...
import { Meta, Robots, StoreTitle } from '@magento/venia-ui/lib/components/Head'
...
import { GET_PRODUCT_DETAIL_QUERY, GET_STORE_CONFIG_DATA } from './product.gql'
...
const Product = props => {
  ...
  return (
    <Fragment>
      ...
      />
      <StoreTitle>{ `${ product.name } - ${ STORE_NAME }` }</StoreTitle>
      <Meta
        name="description"
        content={ product.meta_description }
      />
      <ProductFullDetail product={ product } crhData={ product.crhData || {} }/>
    </Fragment>
  )
}
