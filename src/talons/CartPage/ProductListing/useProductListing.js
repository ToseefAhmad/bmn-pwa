...
export const useProductListing = props => {
  ...
  /**
   * @magento-update
   * @link: https://devdocs.magento.com/guides/v2.4/graphql/caching.html
   * Above guide specifies that cart queries should not be loaded with cache.
   * When fetchPolicy is removed, cart item prices will be loaded over cache and are not correctly returned (1 price for all items...)
   */
  const [
    fetchProductListing,
    { called, data, error, loading }
  ] = useLazyQuery(getProductListing, { fetchPolicy: 'no-cache' })
  ...
  }, [cartId, fetchProductListing, isCartUpdating])
