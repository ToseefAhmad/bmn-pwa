...
export const useBreadcrumbs = props => {
  ...
  // Default to without .html for when the query has not yet returned.
  const categoryUrlSuffix = (data && data.category.url_suffix !== null)
    ? data.category.url_suffix
    : ''
  ...
  return {
    ...
    currentCategoryPath:
      (data && data.category && getPath(data.category.url_path, categoryUrlSuffix)) || '#',
    isLoading: loading,
    hasError: !!error,
